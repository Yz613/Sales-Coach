...
export default function AdminSettingsForm() {
  ...
  useEffect(() => {
    fetch(apiPath("/api/admin/settings"))
      .then(async (res) => {
        ...
        setHasStoredKey(Boolean(data.hasKey));
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Network error while saving";
    ...
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="border-b border-slate-800 pb-5">
        ...
        <p className="text-sm text-slate-400 mt-1">
          Configure the Google Gemini API key that powers live AI call evaluations, and manage your Clerk organization.
        </p>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-6">
        ...
        {saveError && (
          <div className="flex items-center gap-2 rounded-lg border border-rose-500/30 bg-rose-500/10 p-3.5 text-sm text-rose-300">
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/90 p-6 space-y-5">
          <div className="flex items-center gap-2 border-b border-slate-800 pb-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Organization admins have complete access. Organization members are restricted to call uploads and scoring.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 text-xs text-slate-300">
            ...
            {isClerkConfigured && <OrganizationSettingsPanel variant="status" />}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="p-3.5 rounded-lg border border-slate-800 bg-slate-950/60 space-y-2">
                ...
                <div className="font-mono text-[11px] text-slate-400 bg-slate-900 p-2 rounded border border-slate-800">
                  Clerk org role: org:admin (or publicMetadata.role = admin)
                </div>
              </div>
      ...
      {isClerkConfigured && <OrganizationSettingsPanel variant="profile" />}
    </div>
  );
}
